...
if (foundUser) {
  ...
  res.status(200).json({ status: 200, message: "User found", user: foundUser });
} else {
  ...
  if(result.insertedCount === 1) {
    res.status(200).json({ status: 200, message: "User added", user: newUser });
  } else {
...
const getLoopsByUserId = async (req, res) => {
  ...
  if (foundLoops) {
    // If user is found, return success response
    res.status(200).json({ status: 200, message: "Loops found", loops: foundLoops });
  } else {
    // If user is not found, add them to the database
