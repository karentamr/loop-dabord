...
const Recorder = () => {
  ...
  const startRecording = async () => {
    ...
    // Add the selectedAudioDevice as a constraint
    const constraints = {
      audio: {
        deviceId: selectedAudioDevice
          ? { exact: selectedAudioDevice }
          : undefined,
        volume: volume, // set the recording volume dynamically
      },
    };
  ...
  const handleVolumeChange = (event) => {
    const newVolume = event.target.value / 100; // convert the range value to a decimal between 0 and 1
    setVolume(newVolume);
  };
